...
//    out.mPupilThresh = mPupilThresh;
    ...
    //get the threshold in the roi image use ostu method.
    cv::Mat mPupilThresh;
    out.threshold = cv::threshold(mHaarPupil, mPupilThresh, 0, 255, cv::THRESH_OTSU);
    mPupilThresh=255 - mPupilThresh;
    cv::morphologyEx( mPupilThresh,mPupilThresh,cv::MORPH_OPEN,
                  cv::getStructuringElement(cv::MORPH_RECT, cv::Size(haarRadius/10,haarRadius/10)),
                  cv::Point(-1,-1),1,
                  cv::BORDER_CONSTANT,
    ...
    //get the convex hull of the segmented thresholded pupil region.
    ...
    {
        const int padding = 3;
        //padding the roi regon 3 pixels
